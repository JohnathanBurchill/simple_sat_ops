/*
    ...
    the recording sat. The panel itself is utils/sat_globe.c, shared with
...
int main(int argc, char **argv)
{
    ...
    // The globe panel draws its labels with the same font.
    ...
    while (!WindowShouldClose()) {
        ...
        if (strcmp(globe.track_key, s->utc) != 0)
            globe_set_track(&globe, s->utc, s->t_start_ms, s->t_end_ms);
