/*
    ...
    Simple Satellite Operations  src/ipc/sso_audit.c

    Append-only audit log of operator actions: each entry carries a UTC
    timestamp and the invoking Unix user. A background writer thread keeps
    the producer side non-blocking.
