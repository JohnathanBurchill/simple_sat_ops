/*
   ...
   Simple Satellite Operations  src/hw/tr_switch.c
...
// Pull the substring after `needle` (which includes the trailing '=')
// from `line` up to the next space or end-of-line, copy into `out`
// (truncating to cap-1 bytes), and return a pointer to the value
// (inside line) or NULL when the key is absent.
...
// from newer firmware. Anything else (boot banner, log-level ack,
// watchdog notice, debug "TX Sensed!") is silently dropped.
static void parse_line(tr_switch_t *s, const char *line, double t_now)
{
    ...
    if (find_kv(hb, "last_tx_ago_s=", age_buf, sizeof age_buf) != NULL) {
        if (strcmp(age_buf, "never") == 0) {
            // +inf so the UI renders "never" as a placeholder without
...
void tr_switch_pump(tr_switch_t *s, double t_now)
{
    ...
    for (;;) {
        ...
        // exceeds the buffer, drop it and resync on the next newline —
        // the firmware never emits long lines, so this is defensive.
...
int tr_switch_is_stale(const tr_switch_t *s, double t_now)
{
    ...
    if (s->t_last_heartbeat <= 0.0) {
        // Grace window after open: only flag stale once a full
        // stale-window has elapsed without ever seeing a beat.
