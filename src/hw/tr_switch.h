/*
   ...
   Simple Satellite Operations  src/hw/tr_switch.h
...
typedef struct tr_switch {
    ...
    // last_tx_ago_s is from the firmware heartbeat extension; older
    // firmware omits the field and this stays NAN. The literal "never"
    // maps to +infinity so the UI can format it as a placeholder
    // without colliding with a real, very-large age.
    ...
    // Count of heartbeat lines successfully parsed since open.
